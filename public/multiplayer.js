...
(function () {
  ...
  const mpRoomLabel = document.getElementById('mpRoomLabel');
  const mpP1Name = document.getElementById('mpP1Name');
  const mpP2Name = document.getElementById('mpP2Name');
  const mpP1Ready = document.getElementById('mpP1Ready');
  const mpP2Ready = document.getElementById('mpP2Ready');
  const mpReadyBtn = document.getElementById('mpReadyBtn');
  const mpLeaveBtn = document.getElementById('mpLeaveBtn');
  const mpCreateRoom = document.getElementById('mpCreateRoom');
  const mpJoinRoom = document.getElementById('mpJoinRoom');
  ...
  function ensureSocket(){
    if (!ioSocket){
      ...
      ioSocket.on('disconnect', () => {
        inRoom = false; ioSocket = null; opponent.angle = null;
        mpStatus.textContent = 'Verbinding verbroken.';
      });
      // Common match handler (quick or room)
      ...
      ioSocket.on('room:created', (data) => {
        mpStatus.textContent = `Kamer aangemaakt: ${data.code}. Deel deze code.`;
        if (mpRoomCode) mpRoomCode.value = data.code;
      });
      ioSocket.on('room:error', (msg) => {
  ...
  mpCreateRoom?.addEventListener('click', () => {
    ensureSocket();
    ioSocket.emit('room:create', { name: (mpName?.value||'').trim().slice(0,24) });
  });
  mpJoinRoom?.addEventListener('click', () => {
    ensureSocket();
    const code = (mpRoomCode?.value || '').trim().toUpperCase();
    if (!code) { mpStatus.textContent = 'Vul een code in.'; return; }
    ioSocket.emit('room:join', { code, name: (mpName?.value||'').trim().slice(0,24) });
  });
