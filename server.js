...
const rooms = new Map(); // code -> { code, members: [socketId], names: {socketId:name} }
...
io.on('connection', (socket) => {
  ...
  socket.on('room:create', (data={}) => {
    ...
    rooms.set(code, { code, members:[socket.id], names: { [socket.id]: name } });
    socket.join(code);
  ...
  socket.on('room:join', (data={}) => {
    ...
    socket.join(code);
    // match start for both
    const startAt = Date.now() + 3000;
    const seed = Math.floor(Math.random()*1e9);
    const [a,b] = room.members;
    io.to(a).emit('queue:match', { roomId: code, peerName: room.names[b], startAt, seed });
    io.to(b).emit('queue:match', { roomId: code, peerName: room.names[a], startAt, seed });
  });
