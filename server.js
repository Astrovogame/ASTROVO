...
const rooms = new Map();
function getRoom(room){ if(!rooms.has(room)) rooms.set(room, { players: new Map() }); return rooms.get(room); }
function broadcastRoom(roomId, payload){
  const r = rooms.get(roomId); if(!r) return;
  for(const p of r.players.values()) if(p.ws.readyState===1) p.ws.send(JSON.stringify(payload));
}
