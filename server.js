// server.js — Static hosting + WebSocket server for ASTROVO
const path = require('path');
const express = require('express');
const { WebSocketServer } = require('ws');
const { v4: uuidv4 } = require('uuid');

const app = express();
const PORT = process.env.PORT || 3000;

app.use(express.static(path.join(__dirname, 'public')));
app.get('/health', (_req, res) => res.send('ok'));
app.get('/rooms', (_req, res) => {
  const dump = {};
  for (const [roomId, r] of rooms.entries()) {
    dump[roomId] = Array.from(r.players.values()).map(p=>({id:p.id,name:p.name,ready:!!p.ready,score:p.score|0,alive:!!p.alive}));
  }
  res.json(dump);
});
const server = app.listen(PORT, () => {
  console.log(`HTTP listening on http://localhost:${PORT}`);
});
const wss = new WebSocketServer({ server });
console.log('WebSocket server attached (same port).');

const rooms = new Map();
function getRoom(room){
  if(!rooms.has(room)) rooms.set(room, { players: new Map() });
  return rooms.get(room);
}
function roomState(roomId){
  const r = rooms.get(roomId);
  if(!r) return { type:'roomState', players: [] };
  return {
    type:'roomState',
    players: Array.from(r.players.values()).map(p => ({
      id: p.id, name: p.name, ready: !!p.ready, score: p.score|0, alive: !!p.alive
    }))
  };
}
function broadcastRoom(roomId, payload){
  const r = rooms.get(roomId); if(!r) return;
  for(const p of r.players.values()){
    if(p.ws.readyState===1){
      try{ p.ws.send(JSON.stringify(payload)); }catch{}
    }
  }
}

wss.on('connection', (ws)=>{
  const id = uuidv4();
  let currentRoom = null;
  ws.send(JSON.stringify({type:'hello', id}));

  ws.on('message', data=>{
    let msg={}; try{ msg=JSON.parse(data); }catch{}
    if(msg.type==='create' || msg.type==='join'){
      console.log('CREATE_JOIN', msg);
      currentRoom = msg.room;
      const r = getRoom(currentRoom);
      r.players.set(id, {id, name:msg.name||'Player', ready:false, score:0, alive:true, ws});
      broadcastRoom(currentRoom, roomState(currentRoom));
    }
    if(msg.type==='leave'){
      if(currentRoom && rooms.has(currentRoom)){
        rooms.get(currentRoom).players.delete(id);
        broadcastRoom(currentRoom, roomState(currentRoom));
      }
      currentRoom=null;
    }
    if(msg.type==='ready'){
      console.log('READY', {room: currentRoom, from: id, ready: msg.ready});
      if(!currentRoom) return;
      const r = getRoom(currentRoom);
      const p = r.players.get(id); if(!p) return;
      p.ready = !!msg.ready;
      broadcastRoom(currentRoom, roomState(currentRoom));
      if(r.players.size===2){
        const allReady = Array.from(r.players.values()).every(a=>a.ready);
        if(allReady){
          const seed = Math.floor(Math.random()*1e9)|0;
          const startAt = Date.now()+3000;
          console.log('START_BROADCAST', {room: currentRoom, seed, startAt});
          broadcastRoom(currentRoom, {type:'start', t:3, seed, startAt});
          for(const pl of r.players.values()){ pl.ready=false; pl.score=0; pl.alive=true; }
        }
      }
    }
    if(msg.type==='signal'){
      console.log('SIGNAL', {room: currentRoom, from: id, data: msg.data});
      if(!currentRoom) return;
      const r = getRoom(currentRoom);
      const p = r.players.get(id); if(p){
        if(msg.data && msg.data.kind==='score') p.score = msg.data.score|0;
        if(msg.data && msg.data.kind==='dead') p.alive = false;
      }
      broadcastRoom(currentRoom, {type:'broadcast', data: msg.data});
      broadcastRoom(currentRoom, roomState(currentRoom));
    }
    if(msg.type==='requestStart'){
      console.log('REQUEST_START', {room: currentRoom, from: id});
      if(!currentRoom) return;
      const r = getRoom(currentRoom);
      if(r.players.size===2){
        const allReady = Array.from(r.players.values()).every(a=>a.ready);
        if(allReady){
          const seed = Math.floor(Math.random()*1e9)|0;
          const startAt = Date.now()+2000;
          broadcastRoom(currentRoom, {type:'start', t:2, seed, startAt});
          for(const pl of r.players.values()){ pl.ready=false; pl.score=0; pl.alive=true; }
        }
      }
    }
    if(msg.type==='rematch'){
      if(!currentRoom) return;
      broadcastRoom(currentRoom, {type:'broadcast', data:{kind:'rematchAck'}});
    }
  });

  ws.on('close', ()=>{
    if(currentRoom && rooms.has(currentRoom)){
      const r = rooms.get(currentRoom);
      r.players.delete(id);
      if(r.players.size===0){
        rooms.delete(currentRoom);
      }else{
        broadcastRoom(currentRoom, roomState(currentRoom));
      }
    }
  });
});
